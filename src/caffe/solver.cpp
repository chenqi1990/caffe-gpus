...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  for (; iter_ < param_.max_iter(); ++iter_) {
    ...
    if (display) {
      LOG(INFO) <<"Device " << current_device<< " Iteration " << iter_ << ", loss = " << smoothed_loss;
  ...
  if (param_.display() && iter_ % param_.display() == 0) {
    ...
    LOG(INFO) << "Device " << current_device << " Iteration " << iter_ << ", loss = " << loss;
  }
  if (param_.test_interval() && iter_ % param_.test_interval() == 0) {
    TestAll();
  }
...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  //LOG(INFO) << "          Device " << current_device << " Iteration " << iter_
  ...
  if (param_.test_compute_loss()) {
    ...
    LOG(INFO) << "          Device " << current_device << " Test loss: " << loss;
  }
  for (int i = 0; i < test_score.size(); ++i) {
    ...
    //LOG(INFO) << "          Device " << current_device << "     Test net output #" << i << ": " << output_name << " = "
    //    << mean_score << loss_msg_stream.str();
  }
  std::cout << "Device" << current_device << "\t" 
                 << time(NULL) - start << "\t"
                 << this->get_total_iter() << "\t"
                 << test_score[1] / param_.test_iter(test_net_id) << "\t"
                 << test_score[0] / param_.test_iter(test_net_id) << std::endl;
  Caffe::set_phase(Caffe::TRAIN);
}
...
Dtype SGDSolver<Dtype>::GetLearningRate() {
  ...
  if (lr_policy == "fixed") {
    ...
    this->current_step_ = (*(this->total_iter_) ) / this->param_.stepsize();
    rate = this->param_.base_lr() *
        pow(this->param_.gamma(), this->current_step_);
  } else if (lr_policy == "exp") {
    rate = this->param_.base_lr() * pow(this->param_.gamma(), (*(this->total_iter_)) );
  } else if (lr_policy == "inv") {
    rate = this->param_.base_lr() *
        pow(Dtype(1) + this->param_.gamma() * (*(this->total_iter_) ),
            - this->param_.power());
  } else if (lr_policy == "multistep") {
    if (this->current_step_ < this->param_.stepvalue_size() &&
          (*(this->total_iter_)) >= this->param_.stepvalue(this->current_step_)) {
      this->current_step_++;
      LOG(INFO) << "MultiStep Status: Iteration " <<
      (*(this->total_iter_) )<< ", step = " << this->current_step_;
    }
    rate = this->param_.base_lr() *
        pow(this->param_.gamma(), this->current_step_);
  } else if (lr_policy == "poly") {
    rate = this->param_.base_lr() * pow(Dtype(1.) -
        (Dtype((*(this->total_iter_))) / Dtype(this->param_.max_iter())),
        this->param_.power());
  } else if (lr_policy == "sigmoid") {
    rate = this->param_.base_lr() * (Dtype(1.) /
        (Dtype(1.) + exp(-this->param_.gamma() * (Dtype((*(this->total_iter_))) -
...
void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  //   LOG(INFO) << "          Device " << current_device << " Iteration " << this->iter_ << ", lr = " << rate;
  // }
