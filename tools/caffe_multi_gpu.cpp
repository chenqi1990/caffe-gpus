...
DEFINE_int32(gpu_start, -1,
    "The start device-id which multi-gpu model run on.");
DEFINE_int32(gpu_end, -1,
    "The end device-id which multi-gpu model run on.");
...
int train() {
  ...
  if (FLAGS_gpu_start >= 0 && FLAGS_gpu_end >= 0 ) {
    LOG(INFO) << "Use GPUs between device ID " 
                     << FLAGS_gpu_start << " to " << FLAGS_gpu_end;
    Caffe::init(FLAGS_gpu_start,FLAGS_gpu_end); // added by chenqi, 2015/1/6
  ...
  shared_ptr< caffe::ASGDSolver<float> > asgd_solver( 
    new caffe::ASGDSolver<float>(solver_param) );

  if (FLAGS_snapshot.size()) {
    LOG(INFO) << "Resuming from " << FLAGS_snapshot;
    asgd_solver->Resuming(FLAGS_snapshot);
  } else if (FLAGS_weights.size()) {
    LOG(INFO) << "Finetuning from " << FLAGS_weights;
    asgd_solver->Finetuning( FLAGS_weights );
  } else {
    LOG(INFO) << "Solve";
    asgd_solver->Solve();
  }
